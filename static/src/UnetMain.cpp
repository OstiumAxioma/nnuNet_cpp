...
AI_INT  UnetMain::segModelInfer(nnUNetConfig config, CImg<short> input_volume)
{
	...
	// 保存归一化前的数据
	if (saveIntermediateResults && !preprocessOutputPath.empty()) {
		// 创建UnetIO需要的ImageMetadata类型
		::ImageMetadata metadata;
		metadata.origin[0] = imageMetadata.origin[0];
		metadata.origin[1] = imageMetadata.origin[1];
		metadata.origin[2] = imageMetadata.origin[2];
		metadata.spacing[0] = imageMetadata.spacing[0];
		metadata.spacing[1] = imageMetadata.spacing[1];
		metadata.spacing[2] = imageMetadata.spacing[2];
		UnetIO::savePreprocessedData(normalized_volume, preprocessOutputPath, L"before_normalization", metadata);
	}
	...
	if (saveIntermediateResults && !preprocessOutputPath.empty()) {
		...
		UnetIO::savePreprocessedData(normalized_volume, preprocessOutputPath, L"after_normalization_before_resample", metadata);
		// 保存最终的预处理数据
		UnetIO::savePreprocessedData(scaled_input_volume, preprocessOutputPath, L"preprocessed_normalized_volume", metadata);
	}
